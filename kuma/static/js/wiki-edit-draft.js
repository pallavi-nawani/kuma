(function ($, win, doc) {
    ...
    function displayRestoreDraft(startingContent) {
        ...
        if (prevDraft && !draftMatchesStart) {
            ...
            if(draftRevision === pageRevision || isNewTranslation) {
                ...
                // draft of translation that has since been published
                draftType = 'old';
            ...
            $draftDiv.find('.js-restoreLink').on('click', function() {
                if(draftRevision) {
                    // can't use ID because form has duplicate IDs #facepalm
    ...
    function updateDraftState(action) {
        ...
        if(action) {
            ...
            var $updateTime = $draftTime.clone()
                .attr('title', now.toISOString())
                .text(nowString);
            // append elements
    ...
    */
    ...
    function initDrafting() {
        ...
        if (supportsLocalStorage) {
             ...
             *  TODO: siliently delete drafts over 6 months old?
             *  TODO: and drafts with no save-times because those are ancient
             */
